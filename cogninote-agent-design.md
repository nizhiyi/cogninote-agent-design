...
- 模型开放：支持任意 OpenAI-compatible 模型服务
...
第一版推荐直接调用外部 Embedding API，例如：

- OpenAI-compatible embedding API
- Ollama embedding endpoint
- 自建 Python FastAPI embedding 服务
...
模型列表通过 OpenAI-compatible `/models` 接口自动获取。
