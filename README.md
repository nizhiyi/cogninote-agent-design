...
- 桌面交付：支持构建 Windows 桌面程序和 NSIS 安装包，并新增 macOS Apple Silicon `.app` / `.dmg` 独立打包链路。
...
Windows 桌面打包还需要 Rust stable toolchain、MSVC Build Tools 和 WebView2 Runtime。macOS 桌面打包第一版只支持 Apple Silicon，需要 JDK 25 arm64、Rust stable 和 Xcode Command Line Tools。完整说明见 [桌面构建指南](docs/desktop-build-guide.md)。
...
当前项目已完成文档摄入、Lucene 搜索、模型配置、RAG 对话、智能体模型运行时重构、AI 流式 Markdown 渲染、SQLite 聊天记忆、纯模型对话、空白保真的 SSE 流式输出、Windows 桌面打包和 macOS Apple Silicon 独立打包链路的主要闭环。仍需重点补齐：
