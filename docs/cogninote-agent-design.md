...
- 问题抽屉按解析失败、未索引、本地文件缺失和疑似变化分组，并复用同步目录、重建索引和复制路径操作
...
对 documents.source_path 做轻量文件系统探针
  ↓
聚合解析失败、未索引、缺失文件、疑似变化和停用目录
  ↓
返回全库/目录健康快照与建议动作
```

`KnowledgeHealthService` 只比较文件是否存在、是否为普通文件、文件大小和修改时间，不重新计算内容 hash，也不全量 walk 未导入的新文件。新增、修改和删除文件的真实收敛仍由 `POST /api/knowledge-folders/{id}/sync` 显式触发；索引损坏或 Analyzer/Embedding 变化后的修复仍由目录重建或全量重建索引触发。运行记录写入失败只记录 warning，不反向破坏已经完成的导入、同步或重建主流程。
...
- 资料管理页新增全库健康概览、目录健康 badge、问题数量、最近维护记录和问题抽屉。
- 问题抽屉按解析失败、未索引、本地文件缺失和疑似变化分组，并复用目录同步、目录重建和复制路径等显式修复动作。
