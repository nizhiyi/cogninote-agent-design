...
原计划第八阶段的 SQLite 聊天记忆先后被顺延：第八阶段改做多模型配置，第九阶段改做 UI 视觉可读性与主题系统修正，第十阶段改做知识库目录管理，第十一阶段改做智能体模型运行时重构，第十二阶段改做 AI 流式 Markdown 渲染重构，聊天记忆顺延为第十三阶段。第七阶段会先把前端状态模型和 UI 预留成可对接 SQLite 会话的结构，避免后续推倒重来。
...
- “使用知识库”第一版只做全库开关。开启时沿用当前 RAG；关闭时显示纯对话将在第十三阶段接入，不发送假请求。
- 前端会话只做运行期临时状态，刷新页面后不保证恢复。真正跨重启记忆由第十三阶段 SQLite 实现。
