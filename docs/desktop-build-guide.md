...
```powershell
.\scripts\update-release-version.ps1 0.1.11 -WhatIf
```
...
```text
CogniNote-0.1.11-windows-x64-unsigned-portable
CogniNote-0.1.11-windows-x64-unsigned-installer
CogniNote-0.1.11-windows-x64-signed-portable
CogniNote-0.1.11-windows-x64-signed-installer
```
...
```text
CogniNote-0.1.11-macos-arm64-unsigned-app
CogniNote-0.1.11-macos-arm64-unsigned-dmg
CogniNote-0.1.11-macos-arm64-signed-app
CogniNote-0.1.11-macos-arm64-signed-dmg
CogniNote-0.1.11-macos-notarization-logs
```
...
release_tag = v0.1.11-test.1
```

`release_tag` 留空时，unsigned 构建默认发布到 `v0.1.11-test.1`，signed 构建默认发布到 `v0.1.11`。unsigned Release 会标记为 pre-release。Windows 和 macOS 可以分别运行 workflow，并使用同一个 `release_tag`，后运行的平台会把自己的资产追加到同一个 Release 中。

Release 上传的是真实安装文件，不是 Actions artifact 外层 zip。普通用户优先下载 signed 资产；unsigned 资产只给开发者做技术测试：

```text
CogniNote-0.1.11-windows-x64-unsigned-installer.exe
CogniNote-0.1.11-windows-x64-unsigned-portable.zip
CogniNote-0.1.11-macos-arm64-signed.dmg
CogniNote-0.1.11-macos-arm64-signed.app.zip
```
...
```bash
open ./cogniNote-agent-front/src-tauri/target/release/bundle/dmg/CogniNote_0.1.11_aarch64.dmg
```
...
如果 signed DMG 仍提示“已损坏，无法打开”，优先检查 workflow 的 `CogniNote-0.1.11-macos-notarization-logs`、`spctl` 输出，以及 `desktop-backend.log` 中的实际 app 路径。
...
`CogniNoteBackend.exe` 是后端 app-image 的一部分，负责启动 Spring Boot 服务。它不是桌面应用入口。请运行 `cogninote-agent.exe` 或安装 `CogniNote_0.1.11_x64-setup.exe`。
...
如果本地未签名 `.app` 被 Gatekeeper 拦截，可在系统设置中允许运行，仅用于开发验证。若 GitHub Actions 分发产物仍提示“已损坏，无法打开”，通常说明公证或 staple 没成功，先检查 `CogniNote-0.1.11-macos-notarization-logs` artifact 和 workflow 中的 `spctl` 输出。
