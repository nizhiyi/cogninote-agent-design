...
```powershell
.\scripts\update-release-version.ps1 0.1.35 -WhatIf
```
...
- JDK 25 arm64，设置 `JDK_HOME` 或 `JAVA_HOME` 指向包含 `jlink`、`jpackage` 和 JMODs 的完整 JDK。
- Maven 3.9+。
- Node.js 20.19+ 或兼容版本。
- npm。
- Rust stable toolchain 和 Cargo。
...
Windows workflow 使用 `windows-latest`，通过 `actions/setup-java` 设置 JDK 25，通过 `scripts/install-temurin-jmods.ps1` 补装 Temurin JMODs，通过 `actions/setup-node` 设置 Node，通过 `dtolnay/rust-toolchain` 设置 Rust，然后执行：
...
```text
CogniNote-0.1.35-windows-x64-unsigned-portable
CogniNote-0.1.35-windows-x64-unsigned-installer
CogniNote-0.1.35-windows-x64-signed-portable
CogniNote-0.1.35-windows-x64-signed-installer
```
...
```text
CogniNote-0.1.35-macos-arm64-unsigned-app
CogniNote-0.1.35-macos-arm64-unsigned-dmg
CogniNote-0.1.35-macos-arm64-signed-app
CogniNote-0.1.35-macos-arm64-signed-dmg
CogniNote-0.1.35-macos-notarization-logs
```
...
```text
CogniNote-0.1.35-windows-x64-signed-installer.exe
CogniNote-0.1.35-windows-x64-unsigned-installer.exe
```

macOS updater 指向 `.app.tar.gz`：

```text
CogniNote-0.1.35-macos-arm64-signed.app.tar.gz
CogniNote-0.1.35-macos-arm64-unsigned.app.tar.gz
```
...
release_tag = v0.1.35-test.1
```

`release_tag` 留空时，unsigned 构建默认发布到 `v0.1.35-test.1`，signed 构建默认发布到 `v0.1.35`。unsigned Release 会标记为 pre-release。Windows 和 macOS 可以分别运行 workflow，并使用同一个 `release_tag`，后运行的平台会把自己的资产追加到同一个 Release 中。

Release 上传的是真实安装文件，不是 Actions artifact 外层 zip。普通用户优先下载 signed 资产；unsigned 资产只给开发者做技术测试：

```text
CogniNote-0.1.35-windows-x64-unsigned-installer.exe
CogniNote-0.1.35-windows-x64-unsigned-portable.zip
CogniNote-0.1.35-macos-arm64-signed.dmg
CogniNote-0.1.35-macos-arm64-signed.app.zip
CogniNote-0.1.35-macos-arm64-signed.app.tar.gz
```
...
```bash
open ./cogniNote-agent-front/src-tauri/target/release/bundle/dmg/CogniNote_0.1.35_aarch64.dmg
```
...
如果 signed DMG 仍提示“已损坏，无法打开”，优先检查 workflow 的 `CogniNote-0.1.35-macos-notarization-logs`、`spctl` 输出，以及 `desktop-backend.log` 中的实际 app 路径。重点确认 `CogniNote.app/Contents/Resources/backend/CogniNoteBackend.app` 已用同一 Developer ID 证书签名，且发布用 DMG 是由已 staple 的 `CogniNote.app` 重新生成的。
...
`CogniNoteBackend.exe` 是后端 app-image 的一部分，负责启动 Spring Boot 服务。它不是桌面应用入口。请运行 `cogninote-agent.exe` 或安装 `CogniNote_0.1.35_x64-setup.exe`。
...
第 30 阶段的桌面后端会先用 `jlink` 裁剪 custom runtime，再交给 `jpackage --runtime-image`。`jlink` 需要 JDK 的 `jmods/java.base.jmod`；GitHub Actions 通过 `actions/setup-java` 安装的 Temurin JDK 25 可能只带 `java`、`jlink`、`jpackage`，不带单独的 JMODs 目录。

CI 已在 setup-java 后增加 JMODs 补装步骤：

```powershell
.\scripts\install-temurin-jmods.ps1 -Architecture x64
```

```bash
bash ./scripts/install-temurin-jmods-macos.sh
```

补装脚本会下载对应平台的 Temurin JMODs，校验 SHA-256，并把 `JDK_JMODS_DIR` 写入 GitHub Actions 环境。打包脚本优先使用 `JDK_JMODS_DIR`，本地完整 JDK 则继续使用 `$JAVA_HOME/jmods`。
