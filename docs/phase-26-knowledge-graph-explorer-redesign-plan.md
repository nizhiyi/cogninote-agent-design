...
- 关系图使用固定圆环布局，关系方向、关系类型、关系描述和证据强弱不清楚。
...
- 完整图谱只在用户点击“查看”后的全屏弹窗中渲染，关系图弹窗提供筛选栏、大画布和右侧 Inspector。
...
```text
顶部：生成知识图谱 / 已有知识图谱 两个模式按钮
生成知识图谱：三级范围选择（全部 / 目录 / 文件） / 生成 / 取消 / 进度与完成提示
已有知识图谱：可搜索、可按范围类型和视图可用性筛选的摘要清单，条目提供 查看 / 重新生成 / 删除
查看弹窗：按需读取完整 payload；关系图为左侧筛选栏 / 中间 Cytoscape 画布 / 右侧 Inspector
证据抽屉：复用 GraphEvidenceDrawer
```

知识图谱页首屏不自动请求完整 `view` payload，只调用 `GET /api/knowledge-graphs` 展示已有图谱清单。范围选择器只出现在“生成知识图谱”模式中，用于新建或重新生成；真正进入某个历史图谱时，必须由清单条目的“查看”触发 `status/view` 读取，并在全屏弹窗中渲染。生成完成后前端显示完成提示，用户可以立即打开对应图谱。
...
- 删除按钮必须使用危险样式，并在二次确认弹窗中说明“删除图谱节点、关系、证据、视图和运行记录，不删除原始目录或文件”。
- 确认后调用 `DELETE /api/knowledge-graphs`；成功后从清单移除对应 scope，若该 scope 正在弹窗中打开，则清空当前视图并关闭弹窗。
- 正在 `QUEUED` / `RUNNING` 的 scope 后端会拒绝删除，前端展示错误提示。
...
- 已生成图谱清单要展示用户可读名称、路径、节点数、关系数和更新时间，并提供搜索、筛选、查看、重新生成和删除；长路径必须可换行或截断，不能挤压操作按钮。
...
- 验证已有图谱清单的搜索、范围筛选、视图筛选、查看、重新生成和删除可用；删除必须二次确认，成功后清单移除对应 scope，运行中删除显示错误。
