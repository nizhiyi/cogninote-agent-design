...
已生成图谱：全库 / 目录 / 文档级摘要清单，点击“查看”后按需读取完整 payload
主区：当前已打开 scope 的视图预览
全屏弹窗：左侧筛选栏 / 中间 Cytoscape 画布 / 右侧 Inspector
证据抽屉：复用 GraphEvidenceDrawer
```

知识图谱页首屏不自动请求完整 `view` payload，只调用 `GET /api/knowledge-graphs` 展示已生成图谱清单。范围选择器保留给新建或重新生成图谱；真正进入某个历史图谱时，必须由清单条目的“查看”触发 `status/view` 读取。
...
- 已生成图谱清单要展示用户可读名称、路径、节点数、关系数和更新时间；长路径必须可换行或截断，不能挤压操作按钮。
