...
- 按目录和全库聚合解析失败、索引缺失、本地路径不可访问、可能过期、停用目录等状态。
...
| `WARNING` | 有非致命问题，例如部分文件失败、目录停用、少量文档疑似变化。 | 可以使用，但结果可能不完整。 |
| `ERROR` | 目录不可访问、大量解析失败、已解析文档大面积未索引。 | 搜索/RAG 可信度低，需要修复。 |
| `DISABLED` | 目录已停用，不参与搜索/RAG。 | 数据保留，但当前不可检索。 |
...
| `DISABLED_FOLDER` | `WARNING` | 目录 `enabled=false`。 | 启用目录。 |
...
第一版可以只实现前七类，`EMBEDDING_UNCONFIGURED` 和 `GRAPH_STALE` 可作为健康概览中的提示项，不阻断资料管理。
...
知识库工作台保持现有三个 tab，不新增一级导航。资料管理 tab 内新增三块：
...
2. **目录健康列**
   - 每个目录卡片或列表行显示状态 badge。
   - 目录行显示：文档数、失败、未索引、疑似变化、最近操作。
...
- 停用目录仍显示健康，但标记为 `DISABLED`，不参与全库可检索健康评分。
