...
实施状态：已落地。第十九阶段把桌面分发从“产物可生成”推进到“安装、卸载、升级可验证”。macOS unsigned 包只保留为技术测试 artifact，普通用户分发以 signed、notarized、stapled DMG 为准；Windows NSIS 安装器增加安装/卸载钩子，处理旧进程、旧安装目录资源、WebView2 缓存和升级覆盖。

## Summary

本阶段解决三个真实安装问题：macOS 从 GitHub Release 下载 unsigned DMG 后仍可能被 Gatekeeper 判定“已损坏”；Windows 安装新版或卸载后可能因为旧进程、旧安装目录资源或快捷方式残留而继续打开旧版本；桌面 WebView 的持久 HTTP/字节码缓存可能让已安装的新版本继续显示旧前端页面。
...
- Windows NSIS 安装前和卸载后清理 `%LOCALAPPDATA%\com.itqianchen.cogninote\EBWebView` 下的 HTTP、Code Cache、GPU、Service Worker 和 CacheStorage 等缓存目录，保留 `%APPDATA%\CogniNote` 业务数据和 WebView localStorage。
- macOS DMG 拖拽安装没有安装前 hook，WKWebView 缓存处理放到 Tauri 启动阶段：版本变化时清理已知 CogniNote WebView 缓存路径，macOS 14+ 使用版本相关 `data_store_identifier` 隔离数据仓库。
- Spring Boot 为桌面前端入口、SPA 路由和 `/assets/**` 设置 `no-store/no-cache` 响应头；Windows 和 macOS 后端打包脚本在 Maven package 前删除 `target/classes/static`，避免旧 Vite hash 文件残留进新 jar。
...
- 新增 Spring Boot 静态资源缓存头配置：`src/main/java/com/itqianchen/agentdesign/config/StaticResourceCacheConfig.java`。
- Tauri 桌面壳新增 macOS 启动期 WebView 缓存版本标记：`~/Library/Application Support/CogniNote/desktop-webview-version.txt`。
...
- 本阶段不引入自动更新、MSIX/MSI 或 macOS `.pkg`；如果 DMG 覆盖安装仍频繁出问题，后续再评估 `.pkg`。在引入 `.pkg` 前，macOS 无法实现“安装时”清理，只能在应用启动时处理 WKWebView 缓存。
